...
namespace Barcode
{
    internal class BarcodeEAN13 : BarcodeCommon
    {
        private readonly string[] EAN_CodeA = new string[] { "0001101", "0011001", "0010011", "0111101", "0100011", "0110001", "0101111", "0111011", "0110111", "0001011" };
        private readonly string[] EAN_CodeB = new string[] { "0100111", "0110011", "0011011", "0100001", "0011101", "0111001", "0000101", "0010001", "0001001", "0010111" };
        private readonly string[] EAN_CodeC = new string[] { "1110010", "1100110", "1101100", "1000010", "1011100", "1001110", "1010000", "1000100", "1001000", "1110100" };
        private readonly string[] EAN_Pattern = new string[] { "aaaaaa", "aababb", "aabbab", "aabbba", "abaabb", "abbaab", "abbbaa", "ababab", "ababba", "abbaba" };
        ...
        public BarcodeEAN13(string input, bool disableCountryCode = false)
        {
            RawData = input;
        ...
        private void CheckDigit()
        {
            try
            {
                ...
                {
                    if (i % 2 == 0)
                        odd += Convert.ToInt32(rawDataHolder.Substring(i, 1));
                    else
                        even += Convert.ToInt32(rawDataHolder.Substring(i, 1)) * 3;
                }
        ...
        private string Converter()
        {
            ...
            var patterncode = EAN_Pattern[Convert.ToInt32(RawData[0].ToString())];
            var result = "101";
            ...
            while (pos < 6)
            {
                if (patterncode[pos] == 'a')
                    result += EAN_CodeA[Convert.ToInt32(RawData[pos + 1].ToString())];
                if (patterncode[pos] == 'b')
                    result += EAN_CodeB[Convert.ToInt32(RawData[pos + 1].ToString())];
                pos++;
            }
            ...
            pos = 1;
            while (pos <= 5)
                result += EAN_CodeC[Convert.ToInt32(RawData[(pos++) + 6].ToString())];

            var cs = Convert.ToInt32(RawData[RawData.Length - 1].ToString());
